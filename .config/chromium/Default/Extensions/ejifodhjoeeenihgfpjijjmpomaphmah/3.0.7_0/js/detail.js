...
function getDayName_fix(day)
{
    var names = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    return chrome.i18n.getMessage(names[day]);
}
...
function getShortDayName_fix(day)
{
    return getDayName_fix(day).substr(0,3);
}
...
function fillSelectDate()
{
    ...
    $('#dateEndSelect,.dateSelectId').val(end);
    //console.log(start+' '+end);
    $( "#dateStartSelect,#dateEndSelect" ).datepicker( "option", "minDate", new Date(startObject) );
    $( "#dateStartSelect,#dateEndSelect" ).datepicker( "option", "maxDate", new Date() );
    //$('#dateStartSelect').datepicker('setDate', '2012-01-01');
    //$('#dateEndSelect').datepicker('setDate', end);
}
...
function busiestDays()
{
    ...
    for (i in days)
    {
        ...
        output += '<tr rel="' + days[i][0] + '" class="busiest_days_hover"><td><a href="#" data-date="' + days[i][0] + '">' + date.toLocaleDateString() + '</a></td><td>' + getShortDayName_fix(date.getDay()) + '</td><td class="right">' + secondsToHourMinSec(days[i][1]) + '</td></tr>';
    }
...
$(function() {
    ...
    $('#left ul li a').live('click',function(e){
        e.preventDefault();
        
        trackButton('Options','Menu',$(this).attr('data-analytics-id'));
        
        if($(this).attr('id') == 'dashboard'){
            $('#dashboard_new_general').css('display','block');
        }else{
            $('#dashboard_new_general').css('display','none');
        }
        ...
        if($(this).attr('id') == 'alerts_tab'){
            ...
            return;
        }else if($(this).attr('id') == 'menu_premium'){
        ...
        var category = '#right .category[data-selector="' + $(this).attr('id') + '"]';
    ...
    $(".button_share").click(function(){
        shareImage($(this).attr('data-width'),$(this).attr('data-height'),$(this).attr('data-text'));
        trackButton('Options',$(this).parent('div.category').attr('data-selector'),'Facebook-share-chart');
    });
    
    /*var buttons = document.querySelectorAll('#left ul a');
    for(var i = 0; i < buttons.length; i++)
    {
        $(buttons[i]).live('click', function(){
            trackButton('Options','Menu',$(this).attr('data-analytics-id'));
        });
    }*/
    ...
    $('#leftmenu_other_show').click(function(){
        if($('#leftmenu_other').hasClass('none')){
            $('#leftmenu_other').removeClass('none');
        }else{
            $('#leftmenu_other').addClass('none');
        }
    });
...
function renderNewLeftMenu(){
    var menu = getPref('left_menu');
    if(!menu){
        menu = {
            main: [
                {i18n: 'stats_visited', html: '<li><a href="" i18n="stats_visited" data-analytics-id="stats_visited" data-range="true" id="category_visited"></a></li>'},
                {i18n: 'domain_zones', html: '<li><a href="" i18n="domain_zones" data-analytics-id="domain_zones" data-range="true" id="category_zones"></a></li>'},
                {i18n: 'stats_sites', html: '<li><a href="" i18n="stats_sites" data-analytics-id="stats_sites" data-range="true" id="category_sites"></a></li>'},
                {i18n: 'stats_categories', html: '<li class="show_only_in_main_profile"><a href="" i18n="stats_categories" data-analytics-id="stats_categories" data-range="true" id="category_categories"></a></li>'},
                {i18n: 'stats_daily', html: '<li><a href="" i18n="stats_daily" data-analytics-id="stats_daily" data-range="false" id="category_daily"></a></li>'},
            ],
            other: [
                {i18n: 'stats_weekdays', html: '<li><a href="" i18n="stats_weekdays" data-analytics-id="stats_weekdays" data-range="true" id="category_weekdays"></a></li>'},
                {i18n: 'stats_months', html: '<li><a href="" i18n="stats_months" data-analytics-id="stats_months" data-range="false" id="category_months"></a></li>'},
                {i18n: 'stats_time', html: '<li><a href="" i18n="stats_time" data-analytics-id="stats_time" data-range="true" id="category_time"></a></li>'},
                {i18n: 'stats_most', html: '<li><a href="" i18n="stats_most" data-analytics-id="stats_most" data-range="false" id="category_most_visited"></a></li>'},
                {i18n: 'stats_busiest', html: '<li><a href="" i18n="stats_busiest" data-analytics-id="stats_busiest" data-range="false" id="category_busiest"></a></li>'},
                {i18n: '', html: '<li class="display_only_fb_share"><a href="" data-analytics-id="stats_search" data-range="false" id="category_search">Search domains</a></li>'},
                {i18n: '', html: '<li class="display_only_fb_share"><a href="" data-analytics-id="stats_alerts" data-range="false" id="category_alarm">Alerts</a></li>'},
                {i18n: '', html: '<li class="display_only_fb_share"><a href="" data-analytics-id="stats_reports" data-range="false" id="category_reports">Reports</a></li>'}
            ]
        };
        setPref('left_menu',menu);
    }
    var mainElm = $('#leftmenu_main');
    mainElm.empty();
    for(var i in menu.main){
        var html = menu.main[i].html;
        if(menu.main[i].i18n){
            var transl = chrome.i18n.getMessage(menu.main[i].i18n);
            html = html.replace('</a>',transl+'</a>');
        }
        mainElm.append($(html));
    }
    var otherElm = $('#leftmenu_other');
    otherElm.empty();
    for(var i in menu.other){
        var html = menu.other[i].html;
        if(menu.other[i].i18n){
            var transl = chrome.i18n.getMessage(menu.other[i].i18n);
            html = html.replace('</a>',transl+'</a>');
        }
        otherElm.append($(html));
    }
    var edit_menu = $('<div class="edit_items">Edit menu items</div>');
    edit_menu.click(function(){
        $('.leftmenu_modify li a').addClass('modify');
        $('.leftmenu_modify').disableSelection().sortable({
            connectWith: '.leftmenu_modify',
            stop: function(event,ui){
                newLeftMenuMoveItem(ui.item.context.getElementsByTagName('a')[0].getAttribute('id'),ui.item.context.parentElement.getAttribute('id').split('leftmenu_')[1],ui.item.index());
            }
        });
        $(this).css('display','none');
        var edit_close = $('<div class="edit_items">Close edit mode</div>');
        edit_close.click(function(){
            renderNewLeftMenu();
        });
        otherElm.append(edit_close);
        
    });
    otherElm.append(edit_menu);
}

function newLeftMenuMoveItem(id,newSection,newPos){
    var menu = getPref('left_menu');
    var tempItem;
    for(var i in menu.main){
        if(menu.main[i].html.indexOf(id) != -1){
            tempItem = menu.main[i];
            menu.main.splice(i,1);
        }
    }
    for(var i in menu.other){
        if(menu.other[i].html.indexOf(id) != -1){
            tempItem = menu.other[i];
            menu.other.splice(i,1);
        }
    }
    menu[newSection].splice(newPos,0,tempItem);
    setPref('left_menu',menu);
}
...
function initSortable(){
    $('#new_categ_system .categ ul').disableSelection().sortable({
        connectWith: '#new_categ_system .categ ul',
        stop: function(event,ui){
            newSysCategMoveItem(ui.item.context.getAttribute('data-name'),ui.item.context.parentElement.parentElement.getAttribute('data-name'));
        }
    });
}

function renderNewsysCategory(){
    var categories_temp = getOptions().categories;
    var categories = {};
    categories.other = categories_temp.other;
    for(var j in categories_temp){
        if(j != 'other'){
            categories[j] = categories_temp[j];
        }
    }
    var colors = ["#444","#3366cc", "#dc3912", "#ff9900", "#109618", "#990099", "#0099c6", "#dd4477", "#66aa00", "#b82e2e", "#316395", "#994499", "#22aa99","#3366cc", "#dc3912", "#ff9900", "#109618", "#990099", "#0099c6", "#dd4477", "#66aa00", "#b82e2e", "#316395", "#994499", "#22aa99","#3366cc", "#dc3912", "#ff9900", "#109618", "#990099", "#0099c6", "#dd4477", "#66aa00", "#b82e2e", "#316395", "#994499", "#22aa99"];
    var pos = 0;
    var catsElm = $('#new_categ_system');
    catsElm.empty();
    for(var j in categories){
        var categ = categories[j];
        var catElm = $('<div style="background-color:'+colors[pos]+';border-color:'+colors[pos]+';" class="categ'+(j=='other'?' other':'')+'" data-name="'+j+'"></div>');
        pos++;
        catsElm.append(catElm);
        catElm.append('<span class="name">'+j+'</span>');
        var listElm = $('<ul></ul>');
        catElm.append(listElm);
        for(var i in categ){
            var li = $('<li data-name="'+categ[i]+'"></li>');
            listElm.append(li);
            if(j != 'other'){
                var delElm = $('<img class="delete" src="images/remove16.png" alt="" />');
                delElm.click(function(){
                    if(confirm('Are you sure you want to uncategorize this website?')){
                        newSysCategMoveItem($(this).parent('li').children('.title').text(),'other');
                    }
                });
                li.append(delElm);
            }
            var name = $('<span class="title">'+categ[i]+'</span>');
            li.append(name);
        }
    }
    initSortable();
...
function newSysCategMoveItem(domain,newCateg){
    var options = getOptions();
    var categories = options.categories;
    for(var j in categories){
        var categ = categories[j];
    ...
    categories[newCateg].push(domain);
