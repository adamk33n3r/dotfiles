...
for possible_battery_dir in /sys/class/power_supply/BAT*
    if begin; test -d "$possible_battery_dir"; and test -f "$possible_battery_dir/charge_full"; and test -f "$possible_battery_dir/charge_now"; end
        set current_capacity (cat "$possible_battery_dir/charge_now")
        set battery_capacity (cat "$possible_battery_dir/charge_full")
        set -U LAST_BATTERY_LEVEL (math $current_capacity \* 100 \/ $battery_capacity)
    end
end
